...
exercise reported only operational schools, or did not fill in the closure
status field at all. Either way, the source carries no information about
school disruption in Adamawa.

## First pass response (kept here for the audit trail)
...
school disruption source in the pipeline at that point. It surfaced the
data ceiling rather than silently averaging a misleading 0 % into the
composite score.

## Improved response: swap the indicator (current state)

A stronger and more current source was then identified: the **OCHA Joint
Intersectoral Analysis Framework (JIAF) Nigeria 2026** workbook. JIAF
publishes an Education sector severity score (1 to 5) and Education sector
PiN per LGA for every BAY LGA, with **no nulls** in Adamawa. JIAF is the
agreed humanitarian community estimate for the 2026 HNO and is produced
by OCHA in coordination with the Education Cluster.

Indicator 3 in the composite was swapped from iMMAP `pct_closed` to JIAF
...
- iMMAP `total_schools`, `closed_schools` and `pct_closed` are still carried
  through to the popup as **context** so the historical school inventory
  snapshot remains visible, but they no longer influence classification.
...
12 LGAs and 3 (Severe) for 9 LGAs**, which is consistent with the conflict
and displacement signals seen independently in ACLED and DTM.
...
3. **Search for a more authoritative source.** Validate it against the
   problem (zero nulls, 65 of 65 LGA coverage, sector comparable scale).
4. **Swap and document** the change, keeping the previous source as
   context rather than deleting it.

This is the same judgment chain a programme would need under any
re-targeting of the method to a new country.

## Pipeline enforcement

- `scripts/03_clean_schools.py` keeps the `assert ad_closed == 0` check.
  If iMMAP ever publishes a refreshed list with non zero closures in
  Adamawa, the assertion fires and forces a manual review.
